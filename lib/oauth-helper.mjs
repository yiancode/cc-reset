...
function writeEnvFile({ apiKey, tokenData, profile }) {
  ensureDirs();
  const lines = [
    "# Generated by cc-reset login",
    `export ANTHROPIC_API_KEY=${shellEscape(apiKey)}`,
  ];
...
async function commandComplete(args) {
  ...
  const apiKey = await createApiKey(tokenData.access_token);
  writeEnvFile({ apiKey, tokenData, profile });
